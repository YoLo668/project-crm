...
//String company =  request.getParameter("company");
	/*
	pageContext作用：
		1、当做页面的域对象,getAttribute/setAttribute
		2、可以随时随处的变成其它的域作用对象来使用
		3、pageContexe可以取得另外8个内置对象
	 */
	/*pageContext.setAttribute("str1","123",pageContext.SESSION_SCOPE);
...
<script type="text/javascript">
	$(function(){
		...
		$("#convertBtn").click(function () {
			...
			请求结束后,最终响应回线索的列表页
			根据为客户创建交易的复选框有没有选中,来判断是否需要创建交易
			 */
			if ($("#isCreateTransaction").prop("checked")){
				// alert("需要创建交易")
				//需要创建交易,除了要为后台创建clueId之外还得为后台传递交易表单中的信息,金额,预计成交日期,交易名称,阶段,市场活动源(id)
				//window.location.href = "workbench/clue/convert.do?clueId=${param.id}&money";

				//以上传递参数的方式很麻烦,而且表单一旦扩充,挂载的参数有可能超出浏览器地址栏的上限
				//想到使用提交交易表单的形式来发出本次的传统请求
